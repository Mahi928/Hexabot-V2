...
void rotate_points(float deg,coordinate* start_pt, coordinate* p1_pt, coordinate* p2_pt, coordinate* end_pt){
  ...
  float x_x;
  x_x=start_pt->x;
  start_pt->x=start_pt->x*cos_theta-(start_pt->y-180.)*sin_theta;
  start_pt->y=x_x*sin_theta+(start_pt->y-180.)*cos_theta;
  start_pt->y+=180.;
  x_x=p1_pt->x;
  p1_pt->x=p1_pt->x*cos_theta-(p1_pt->y-180.)*sin_theta;
  p1_pt->y=x_x*sin_theta+(p1_pt->y-180.)*cos_theta;
  p1_pt->y+=180.;
  x_x=p2_pt->x;
  p2_pt->x=p2_pt->x*cos_theta-(p2_pt->y-180.)*sin_theta;
  p2_pt->y=x_x*sin_theta+(p2_pt->y-180.)*cos_theta;
  p2_pt->y+=180.;
  x_x=end_pt->x;
  end_pt->x=end_pt->x*cos_theta-(end_pt->y-180.)*sin_theta;
  end_pt->y=x_x*sin_theta+(end_pt->y-180.)*cos_theta;
  end_pt->y+=180.;
  return;
}
...
void generate_trajec(float deg, float height) {
  ...
  // {
  //   i++;
  //   set[1]->go_to(start_pt.x, start_pt.y, start_pt.z);
  //   delay(1000);
  //   set[1]->go_to(end_pt.x, end_pt.y, end_pt.z);
  //   delay(1000);
  // }
  ...
  rotate_points(240,&start_pt,&p1_pt,&p2_pt,&end_pt);
  Serial.println("Rotation for 2 done");
  ...
  while(iteration <11){
    ...
    // Serial.println(sizeof(bz_pts[0]));
    for(int i = 0; i<11; i++){
      ...
      // Serial.println(i);
      set[2]->go_to(bz_pts[1][i][0], bz_pts[1][i][1], bz_pts[1][i][2]);
      set[5]->go_to(lin_pts[1][10-i][0], lin_pts[1][10-i][1], lin_pts[1][10-i][2]);
      // Serial.println(i);
      set[4]->go_to(bz_pts[2][i][0], bz_pts[2][i][1], bz_pts[2][i][2]);
      set[1]->go_to(lin_pts[2][i][0], lin_pts[2][i][1], lin_pts[2][i][2]);
      delay(10);
    }
    // delay(1000);
    for(int i = 0; i<11; i++){
      set[3]->go_to(bz_pts[0][10-i][0], bz_pts[0][10-i][1], bz_pts[0][10-i][2]);
      set[0]->go_to(lin_pts[0][i][0], lin_pts[0][10-i][1], lin_pts[0][10-i][2]);

      set[2]->go_to(lin_pts[1][i][0], lin_pts[1][i][1], lin_pts[1][i][2]);
      set[5]->go_to(bz_pts[1][10-i][0], bz_pts[1][10-i][1], bz_pts[1][10-i][2]);

      set[4]->go_to(lin_pts[2][i][0], lin_pts[2][i][1], lin_pts[2][i][2]);
      set[1]->go_to(bz_pts[2][i][0], bz_pts[2][i][1], bz_pts[2][i][2]);
      delay(10);
    }
    // delay(1000);
  }  
...
void gen_bz_pts(coordinate start_pt, coordinate p1_pt, coordinate p2_pt, coordinate end_pt, int num_points, int leg_id)
{
  ...
  for (int t = 1; t < num_points+1 ; ++t) {
        ...
        x = (pow(1 - t_normalized, 3) * start_pt.x)
                   ...
                   + (3 * (1 - t_normalized) * pow(t_normalized, 2) *p1_pt.x)
...
void myfunc()
{
  ...
  // for(int i=0; i<11; i++)
  // {
  //   set[1]->go_to(lin_pts[1][i][0], lin_pts[1][i][1], lin_pts[1][i][2]);
  // }
...
void loop() {
  ...
  //     float z_ht = 100.;
      // generate_trajec(deg, z_ht);
  // }
  // set[3]->go_to(-115.10, 118.20,10.0);
  // delay(3000);
  // set[3]->go_to(0.0, 180.0, -100.0);
  // delay(3000);
}